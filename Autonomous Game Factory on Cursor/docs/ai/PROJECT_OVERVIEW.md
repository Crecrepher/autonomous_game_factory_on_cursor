...
- **타겟**: 모든 장르의 Playable Ad (2D / 3D / 아이소메트릭 / 탑다운 / 사이드뷰 등 제한 없음)
- **엔진**: Unity (C#)
- **네임스페이스**: `Game`
- **플랫폼**: 모바일 (iOS / Android)

이 프레임워크는 특정 장르에 종속되지 않는다. 기획서가 첨부되면 해당 게임의 장르·메카닉에 맞는 모듈을 동적으로 생성한다.
게임 기획 상세(게임플레이 루프, 메카닉, 경제, 연출 등)는 별도 기획 문서에서 관리한다. 이 문서는 **개발 파이프라인 아키텍처**에 집중한다.
